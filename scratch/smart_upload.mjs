...
async function smartUpload() {
  ...
  console.log("Listing existing files on B2...");
  ...
  if (!listRes.ok) {
    throw new Error(`B2 List File Names Failed: ${listRes.status} ${await listRes.text()}`);
  }
  ...
  if (listData.files) {
    listData.files.forEach(f => {
      let name = decodeURIComponent(f.fileName);
      remoteFilesMap.set(name, {
        size: f.contentLength,
        sha1: f.contentSha1
      });
    });
  }
  console.log(`Found ${remoteFilesMap.size} files in B2 bucket.`);
  ...
  console.log(`Found ${localFiles.length} local files under NOTES/ directory.`);
  ...
  for (const filePath of localFiles) {
    ...
    const remote = remoteFilesMap.get(b2Key);
    if (remote && remote.sha1 === sha1 && remote.size === fileContent.length) {
      console.log(`[SKIP] "${b2Key}" already exists and matches on B2.`);
      continue;
    }

    console.log(`\n[UPLOAD] "${filePath}" (${(fileContent.length / (1024 * 1024)).toFixed(2)} MB) to B2 as key: "${b2Key}"...`);
  ...
  console.log("\nSmart upload process completed!");
}

smartUpload().catch(err => {
  console.error("Smart upload process crashed:", err);
});
