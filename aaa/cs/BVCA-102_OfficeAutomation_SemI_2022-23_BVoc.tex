...
about 750 words each. (Any two) 15x 2=30

\medskip\noindent\textbf{Question 1.} (a) What is Artificial Intelligence ? Discuss five
applications of Artificial Intelligence.
\begin{parts}
  \item What are the challenges faced by Artificial
  Intelligence ?
\end{parts}

\medskip\noindent\textbf{Question 2.} (a) What are agents and environment ? What is an
Ideal Rational Agent ?
\begin{parts}
  \item Discuss different types of agents in Artificial
  Intelligence.
  P. T.O.
\end{parts}

\medskip\noindent\textbf{Question 3.} (a) What are Heuristic Search techniques ? Discuss
Hill Climbing Algorithm. |
\begin{parts}
  \item How Steepest Ascent Hill Climbing algorithm
  differs from Hill Climbing algorithm ? |
\end{parts}
...
\begin{parts}
  ...
  \begin{tikzpicture}[>=stealth, every node/.style={draw, circle, minimum size=8mm, inner sep=0pt}]
    ...
    \draw[->, thick] (S) -- node[below] {10} (G);
    \draw[->, thick] (A) -- node[above left] {2} (B);
    \draw[->, thick] (A) -- node[below left] {1} (C);
    \draw[->, thick] (B) -- node[above] {5} (D);
    \draw[->, thick] (C) -- node[below right] {3} (D);
    \draw[->, thick] (C) -- node[below] {4} (G);
    \draw[->, thick] (D) -- node[above right] {2} (G);
  \end{tikzpicture}
  \end{center}
