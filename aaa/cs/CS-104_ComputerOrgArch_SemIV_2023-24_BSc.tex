...
\hypersetup{colorlinks=true,urlcolor=black,linkcolor=black,
  pdftitle={CS-104: Computer Org \& Arch -- BHU PYQ}}
...
\fancyhead[R]{\small   \textit{CS-104: Computer Org \& Arch}}
...
\begin{center}
  ...
  {\large\bfseries Paper: CS-104 --- Computer Org \& Arch}\\[4pt]
...
\noindent   \textit{Instructions: Answer five questions in all including Question No. 1, which is compulsory. Answers should be brief and to the point and may be supplemented with neat sketches, wherever required. Figures on the right-hand side margin indicate maximum marks for each question.}
...
\begin{parts}
  \item A cache memory has 8 cache lines (0--7). The memory block requests are in the order:
  \[ 4, 3, 25, 8, 19, 6, 25, 8, 16, 35, 45, 22, 8, 3, 16, 25, 7 \]
  Determine:
  
\begin{parts}
    \item Which cache line will have memory block 7 if fully associative mapping with LRU policy is used. Also, calculate the hit ratio and miss ratio. \pts{3.5}
    \item Which cache line will have memory block 7 if direct mapping is used. Also, calculate the hit ratio and miss ratio. \pts{3.5}
  \end{parts}
...
\begin{parts}
  ...
  \item What is an instruction cycle, and write the phases of the Instruction cycle? \pts{4}
\end{parts}
...
\begin{parts}
  ...
  \item Write the difference between Static RAM and Dynamic RAM. \pts{4}
\end{parts}
...
\begin{parts}
    \item How many chips are required? \pts{1}
    \item How many lines of the address bus must be used to access 4096 bytes of memory? \pts{1}
    \item How many of lines in (ii) will be common to all chips? \pts{1}
    \item How many lines must be decoded for chip select? Specify the size of decoders. \pts{2}
  \end{parts}
