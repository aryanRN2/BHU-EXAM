...
\bigskip
\medskip\noindent\textbf{Question 1.} Write a brief note on the estimation of sample size in the case of simple random sampling, addressing any two methods known to you. For a population of size $N = 430$, we roughly know that $\bar{Y} = 19$ and $S^2 = 85.6$. With SRS, what should be the size of the sample to estimate $\bar{Y}$ with a margin of error of $10\%$ of $\bar{Y}$, apart from a $1$ in $20$ chance? \pts{14}
...
\begin{parts}
  \item Find the values of $n_1/n$ and $n_2/n$ that minimize the total cost for a given value of $V(\bar{y}_{st})$.
  \item Find the sample size required, under this optimum allocation, to make $V(\bar{y}_{st}) = 1$, if fpc is ignored.
