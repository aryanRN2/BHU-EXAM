...
\begin{parts}
  ...
  \item For a distribution, the mean is 10, variance is 16, $\gamma_1 = +1$, $eta_2 = 4$. Obtain the first four moments about the origin. \pts{7}
\end{parts}
...
\begin{parts}
  ...
  \item If $A_1, A_2, \dots, A_n$ be any $n$ mutually exclusive events with $P(A_i) > 0$ for all $i = 1, 2, \dots, n$, and let $A$ be any event such that $A \subset igcup_{i=1}^n A_i$ with $P(A) > 0$, then find the value of $P(A_i | A)$ for all $i = 1, 2, \dots, n$. \pts{7}
\end{parts}
...
\begin{parts}
  ...
  $$f(x) = 
\begin{cases} ax, & 0 \le x \le 2 \\ a(3-x), & 2 < x < 3 \\ 0, &  \text{elsewhere} \end{cases}$$
  ...
  $$f(x, y) = 
\begin{cases} 8xy, & 0 < x < 1, \ 0 < y < x \\ 0, &  \text{otherwise} \end{cases}$$
...
\begin{parts}
  ...
  $$f(x, y) = 
\begin{cases} e^{-(x+y)}, & x > 0, \ y > 0 \\ 0, &  \text{elsewhere} \end{cases}$$
