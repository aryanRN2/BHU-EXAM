...
\begin{parts}
  ...
  \]\pts{7}
  \item Differentiate between mixed strategy and pure strategy in game theory. Let $(a_{ij})$ be the $m \times n$ payoff matrix for a two-person zero-sum game. If $\underline{v}$ denotes the maximin value and $\bar{v}$ denotes the minimax value of the game, then show that $\underline{v} \leq \bar{v}$. \pts{7}
\end{parts}
