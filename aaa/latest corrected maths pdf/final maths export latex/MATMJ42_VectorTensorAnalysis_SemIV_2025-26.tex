...
\begin{parts}
  ...
  \item Write down the expression for the Christoffel symbols of first and second kind $[pq,r]$ and $\begin{Bmatrix} s \\ pq \end{Bmatrix}$ respectively, where $p,q,r,s$ denotes indices.\pts{2}
...
\begin{parts}
  ...
  \item Suppose that $T^i$ is a contravariant vector in $\mathbb{R}^2$ and that $(T^i) = (x^2, x^1)$ in the $(x^i)$ system. Calculate $(\bar{T}^i)$ in the $(\bar{x}^i)$ system under the change of coordinate $\bar{x}^1 = (x^2)^2 \ne 0, \bar{x}^2 = x^1 x^2$.\pts{5}
\end{parts}
\medskip\rule{\linewidth}{0.2pt}
